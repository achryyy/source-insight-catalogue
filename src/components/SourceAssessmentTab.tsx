...
import { Plus, Edit, Trash2, Search, CheckCircle, Clock, AlertCircle, RefreshCw, Activity, ArrowRight, Database, FileText } from 'lucide-react';
import { toast } from 'sonner';

export const SourceAssessmentTab = () => {
  ...
  const handleMigrateToLiveCatalogue = async (source: any) => {
    setMigratingSourceId(source.id);
    ...
    setMigratingSourceId(null);
    toast.success(`${source.source_name} migrated to Live Source Catalogue successfully!`);
  };

  const handleMigrateToBoth = async (source: any) => {
    setMigratingSourceId(source.id);
    ...
    setMigratingSourceId(null);
    toast.success(`${source.source_name} migrated to both ADIP Datasets and Live Source Catalogue successfully!`);
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Source Assessment</h2>
          <p className="text-muted-foreground mt-2">
            Manage and track your source assessment with comprehensive filtering and approval workflow
          </p>
        </div>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button
                        ...
                        className="shadow-sm bg-purple-50 hover:bg-purple-100 text-purple-700 border-purple-200 text-xs px-2"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleMigrateToLiveCatalogue(source)}
                        disabled={migratingSourceId === source.id}
                        className="shadow-sm bg-green-50 hover:bg-green-100 text-green-700 border-green-200 text-xs px-2"
                      >
                        {migratingSourceId === source.id ? (
                          <RefreshCw className="h-3 w-3 animate-spin" />
                        ) : (
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleMigrateToBoth(source)}
                        disabled={migratingSourceId === source.id}
                        className="shadow-sm bg-blue-50 hover:bg-blue-100 text-blue-700 border-blue-200 text-xs px-2"
                      >
                        {migratingSourceId === source.id ? (
                          <RefreshCw className="h-3 w-3 animate-spin" />
                        ) : (
